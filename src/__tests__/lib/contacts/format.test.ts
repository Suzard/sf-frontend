...
describe("addressLine", () => {
  ...
  it("pairs the state with the postal code", () => {
    ...
    expect(addressLine(contact.addresses[0])).toBe("1 Market St, San Francisco, CA 94105, USA");
  });

  it("returns null when there is no address at all", () => {
    expect(addressLine({ address: null, city: null, state: null, country: null, postal_code: null })).toBeNull();
  });
});

describe("primaryAddressLine", () => {
  it("includes the type and first non-empty address", () => {
    const contact = makeContact({
      addresses: [
        { id: 1, type: "Home", address: null, city: null, state: null, postal_code: null, country: null },
        { id: 2, type: "Work", address: "NASA", city: "Hampton", state: null, postal_code: null, country: "USA" },
      ],
    });
    expect(primaryAddressLine(contact)).toBe("Work: NASA, Hampton, USA");
  });
});
