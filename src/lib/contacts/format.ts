...
/** First non-empty address, as a compact fallback for list views. */
export function primaryAddressLine(contact: Contact): string | null {
  for (const address of contact.addresses) {
    const line = addressLine(address);
    if (line) return `${address.type}: ${line}`;
  }
  return null;
}
